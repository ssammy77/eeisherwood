...
const seriesSchema = generateSeriesSchema({
  name: 'Build A Better Apocalypse',
  description: '3-book EMP survival series with a twist. Frank Douglas has prepared his whole life for the apocalypse, but nothing prepared him for what\'s coming next.',
  bookCount: 3,
...
const breadcrumbSchema = generateBreadcrumbSchema([
  ...
  { name: 'Build A Better Apocalypse', url: canonicalUrl(seriesUrl) },
])

export default function BuildBetterApocalypsePage() {
  return (
    <>
      ...
      <div className="min-h-screen font-sans antialiased bg-background text-foreground">
        ...
        <section className="px-6 py-16 md:py-24 max-w-4xl mx-auto">
          ...
          <p className="text-xl text-muted-foreground leading-relaxed mb-8 max-w-3xl">
            You can plan for any apocalypse. But can you plan for all of them? Frank Douglas has prepared his whole life for the end of the world — and it still wasn't enough. The first of 3 books in a new standalone series.
          </p>
          <a href="https://www.amazon.com/gp/product/B0F2SJ2SRJ" target="_blank" rel="noopener noreferrer"
          ...
          <div className="space-y-8">
            {books.map((book) => (
              <div key={book.num} id={`book-${book.num}`}
                   className="border border-border rounded-2xl p-6 md:p-8 bg-card hover:border-emerald-600/30 transition-colors">
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <Link href={`${seriesUrl}/${book.num}`}
                       className="inline-flex items-center px-5 py-2 text-sm font-semibold text-emerald-600 border border-emerald-600 rounded-lg hover:bg-emerald-600/10 transition-colors ml-2">
                      Full Details →
                    </Link>
                  </div>
                </div>
