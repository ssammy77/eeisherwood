...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const book = sirensBooks.find(b => b.num === bookNum)
  ...
  return {
    ...
    openGraph: { url: canonicalUrl(`/sirens-of-the-zombie-apocalypse/${num}`), },
    alternates: { canonical: canonicalUrl(`/sirens-of-the-zombie-apocalypse/${num}`) },
  }
}

export async function generateStaticParams() {
  return sirensBooks.map(book => ({ num: String(book.num) }))
}

export default async function SirensBookPage({ params }: Props) {
  const { num } = await params
  const bookNum = parseInt(num)
  const book = sirensBooks.find(b => b.num === bookNum)
  ...
  const seriesColor = '#dc2626'
  const bookIndex = sirensBooks.findIndex(b => b.num === bookNum)
  const prevBook = bookIndex > 0 ? sirensBooks[bookIndex - 1] : null
  const nextBook = bookIndex < sirensBooks.length - 1 ? sirensBooks[bookIndex + 1] : null
    ...
    <div className="min-h-screen bg-background">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBookSchema({
          ...
          image: `/images/book-sirens${book.num}.jpg`,
          author: 'EE Isherwood',
      ...
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbSchema([
          ...
          { name: 'Sirens of the Zombie Apocalypse', url: `https://eeisherwood.com${seriesUrl}` },
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <div className="flex items-center gap-3 text-sm text-muted-foreground mb-8">
          ...
          <span>›</span>
          <Link href={seriesUrl} className="hover:text-foreground transition-colors">Sirens of the Zombie Apocalypse</Link>
          <span>›</span>
        ...
        <div className="flex justify-between items-center">
          {prevBook ? (
            <Link
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:border-red-600/50 transition-colors"
            >
