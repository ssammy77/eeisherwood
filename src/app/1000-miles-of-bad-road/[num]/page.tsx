...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const book = thousandMilesBooks.find(b => b.num === bookNum)
  ...
  return {
    ...
    openGraph: { url: canonicalUrl(`/1000-miles-of-bad-road/${num}`), },
    alternates: { canonical: canonicalUrl(`/1000-miles-of-bad-road/${num}`) },
  }
}

export async function generateStaticParams() {
  return thousandMilesBooks.map(book => ({ num: String(book.num) }))
}

export default async function ThousandMilesBookPage({ params }: Props) {
  const { num } = await params
  const bookNum = parseInt(num)
  const book = thousandMilesBooks.find(b => b.num === bookNum)
  ...
  const seriesColor = '#ea580c'
  const bookIndex = thousandMilesBooks.findIndex(b => b.num === bookNum)
  const prevBook = bookIndex > 0 ? thousandMilesBooks[bookIndex - 1] : null
  const nextBook = bookIndex < thousandMilesBooks.length - 1 ? thousandMilesBooks[bookIndex + 1] : null
  const seriesUrl = '/1000-miles-of-bad-road'
    ...
    <div className="min-h-screen bg-background">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBookSchema({
          ...
          isbn13: book.isbn13,
          asin: book.asin,
          image: `/images/book-1000mi${book.num}.jpg`,
          author: 'EE Isherwood',
      ...
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbSchema([
          ...
          { name: '1000 Miles of Bad Road', url: `https://eeisherwood.com${seriesUrl}` },
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <div className="flex items-center gap-3 text-sm text-muted-foreground mb-8">
          ...
          <span>›</span>
          <Link href={seriesUrl} className="hover:text-foreground transition-colors">1000 Miles of Bad Road</Link>
          <span>›</span>
        ...
        <div className="flex flex-col md:flex-row gap-10 md:gap-16">
          <div className="flex-shrink-0">
            <img
              src={`/images/book-1000mi${book.num}.jpg`}
          ...
          <div className="flex-1">
            ...
            <h1 className="text-4xl font-bold font-serif mb-4">{book.title.replace('1000 Miles of Bad Road ', '')}</h1>
        ...
        <div className="flex justify-between items-center">
          {prevBook ? (
            <Link
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:border-orange-600/50 transition-colors"
            >
              ← Book {prevBook.num}: {prevBook.title.replace('1000 Miles of Bad Road ', '')}
            </Link>
          ) : <div />}
          {nextBook ? (
            <Link
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:border-orange-600/50 transition-colors"
            >
              Book {nextBook.num}: {nextBook.title.replace('1000 Miles of Bad Road ', '')} →
            </Link>
          ) : <div />}
        </div>
      </main>
