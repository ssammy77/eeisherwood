...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const book = impactBooks.find(b => b.num === bookNum)

  if (!book) return { title: 'Book Not Found' }

  return {
    title: `${book.title} — Impact Series — EE Isherwood`,
    description: book.shortBlurb,
    openGraph: { url: canonicalUrl(`/impact/${num}`), },
    alternates: { canonical: canonicalUrl(`/impact/${num}`) },
  }
}

export async function generateStaticParams() {
  return impactBooks.map(book => ({ num: String(book.num) }))
}

export default async function ImpactBookPage({ params }: Props) {
  const { num } = await params
  const bookNum = parseInt(num)
  const book = impactBooks.find(b => b.num === bookNum)
  ...
  const seriesColor = '#0891b2'
  const bookIndex = impactBooks.findIndex(b => b.num === bookNum)
  const prevBook = bookIndex > 0 ? impactBooks[bookIndex - 1] : null
  const nextBook = bookIndex < impactBooks.length - 1 ? impactBooks[bookIndex + 1] : null
  const seriesUrl = '/impact'
    ...
    <div className="min-h-screen bg-background">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbSchema([
          ...
          { name: 'Impact', url: `https://eeisherwood.com${seriesUrl}` },
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <div className="flex items-center gap-3 text-sm text-muted-foreground mb-8">
          ...
          <span>›</span>
          <Link href={seriesUrl} className="hover:text-foreground transition-colors">Impact</Link>
          <span>›</span>
        ...
        <div className="flex flex-col md:flex-row gap-10 md:gap-16">
          <div className="flex-shrink-0">
            <img
              src={`/images/book-impact${book.num}.jpg`}
        ...
        <div className="flex justify-between items-center">
          {prevBook ? (
            <Link
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:border-cyan-600/50 transition-colors"
            >
