...
const breadcrumbSchema = generateBreadcrumbSchema([
  ...
  { name: 'Impact', url: canonicalUrl(seriesUrl) },
])
      ...
      <div className="min-h-screen font-sans antialiased bg-background text-foreground">
        ...
        <section className="px-6 py-16 md:py-24 max-w-4xl mx-auto">
          ...
          <p className="text-xl text-muted-foreground leading-relaxed mb-8 max-w-3xl">
            When trillion-dollar rocks fall from space, middle America has to survive without any help from the people who caused it. Six books of asteroid impact fiction, originally co-published with Mike Kraus.
          </p>
          <a href="https://www.amazon.com/gp/product/B0C66J5ZQM" target="_blank" rel="noopener noreferrer"
          ...
          <div className="space-y-8">
            {books.map((book) => (
              <div key={book.num} id={`book-${book.num}`}
                   className="border border-border rounded-2xl p-6 md:p-8 bg-card hover:border-cyan-600/30 transition-colors">
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <Link href={`${seriesUrl}/${book.num}`}
                       className="inline-flex items-center px-5 py-2 text-sm font-semibold text-cyan-600 border border-cyan-600 rounded-lg hover:bg-cyan-600/10 transition-colors ml-2">
                      Full Details →
                    </Link>
                  </div>
                </div>
