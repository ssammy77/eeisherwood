...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const book = minusAmericaBooks.find(b => b.num === bookNum)
...
export async function generateStaticParams() {
  return minusAmericaBooks.map(book => ({ num: String(book.num) }))
}

export default async function MinusAmericaBookPage({ params }: Props) {
  const { num } = await params
  const bookNum = parseInt(num)
  const book = minusAmericaBooks.find(b => b.num === bookNum)
  ...
  const seriesColor = '#d97706'
  const bookIndex = minusAmericaBooks.findIndex(b => b.num === bookNum)
  const prevBook = bookIndex > 0 ? minusAmericaBooks[bookIndex - 1] : null
  const nextBook = bookIndex < minusAmericaBooks.length - 1 ? minusAmericaBooks[bookIndex + 1] : null
  const seriesUrl = `/minus-america`
    ...
    <div className="min-h-screen bg-background">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBookSchema({
          ...
          image: `/images/book-ma${book.num}.jpg`,
          author: 'EE Isherwood',
      ...
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbSchema([
          ...
          { name: 'Minus America', url: `https://eeisherwood.com${seriesUrl}` },
        ...
        <div className="flex flex-col md:flex-row gap-10 md:gap-16">
          ...
          <div className="flex-1">
            ...
            <a 
              ...
              className="inline-flex items-center px-8 py-3 rounded-lg font-semibold text-white bg-amber-600 hover:bg-amber-700 transition-colors mb-10"
            >
              Get on Amazon →
            </a>
          </div>
        ...
        <div className="flex justify-between items-center">
          {prevBook ? (
            <Link 
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:border-amber-600/50 transition-colors"
            >
